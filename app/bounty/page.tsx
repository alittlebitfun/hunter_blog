  ...
  en: [
    {
      ...
      date: "2024-01-15",
      excerpt:
        "A revolutionary AI algorithm has vanished from our secure servers. Intelligence suggests it was stolen by a rogue developer known only as 'The Phantom Coder.'",
    ...
    },
    {
      ...
      author: "MI-Code",
      date: "2024-01-08",
      excerpt: "Trade secrets are being leaked to competitors. We suspect a mole within the organization.",
    ...
    },
    {
      ...
      date: "2024-01-05",
      excerpt: "A major hospital network has been hit by ransomware. Patient data is encrypted and systems are down.",
      tags: ["Ransomware", "Healthcare", "Emergency", "Decryption"],
      status: "CRITICAL",
    },
    {
      id: 6,
      title: "Social Engineering Attack Analysis",
      reward: "$2,800",
      difficulty: "MEDIUM",
      location: "Chicago",
      timeLimit: "8 Days",
      ...
      date: "2024-01-03",
      excerpt:
        "Employees are falling victim to sophisticated phishing campaigns. We need to identify the attack vectors.",
      tags: ["Social Engineering", "Phishing", "Training", "Analysis"],
      status: "ACTIVE",
    },
  ],
  zh: [
...
function BountyContent() {
  ...
  // 根据语言获取悬赏数据
  const bountyPosts = bountyPostsData[locale as keyof typeof bountyPostsData] || bountyPostsData.en
  ...
  const getRankTitle = (killCount: number) => {
    switch (killCount) {
      ...
      default:
        return { title: "HUNTER", subtitle: "猎人", color: "text-gray-400" }
    }
  }
      ...
      <div className="relative z-10 container mx-auto px-4 py-8">
        ...
        <div className="grid gap-6">
          {bountyPosts.map((post, index) => (
            ...
            >
              <Link href={getLocalizedPath(`/bounty/${post.id}`)}>
        ...
        <div className="mt-12 grid grid-cols-1 md:grid-cols-4 gap-4 font-mono">
          <div className="bg-black/50 border border-red-600 p-4 text-center">
            <div className="text-2xl font-bold text-red-400">{bountyPosts.length}</div>
          ...
          <div className="bg-black/50 border border-orange-600 p-4 text-center">
            <div className="text-2xl font-bold text-orange-400">
              {bountyPosts.filter((p) => p.status === "URGENT" || p.status === "CRITICAL").length}
            </div>
          ...
          <div className="bg-black/50 border border-green-600 p-4 text-center">
            <div className="text-2xl font-bold text-green-400">
              {locale === "zh" ? "¥" : "$"}
              {Math.max(...bountyPosts.map((p) => Number.parseInt(p.reward.replace(/[¥$,]/g, "")))).toLocaleString()}
            </div>
